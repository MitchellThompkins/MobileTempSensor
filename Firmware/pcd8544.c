...
/*
 * Name         :  LcdDeinit
 * Description  :  Performs MCU SPI & LCD controller Deinitialization.
 * Argument(s)  :  None.
 * Return value :  None.
 */
void LcdDeinit ( void )
{
    /* Pull-up on reset pin. */
    //LCD_PORT &= ~(_BV ( LCD_RST_PIN ));
	SPCR = 0x10; //Disable MOSI register first.
	LCD_PORT &= ~(_BV(LCD_DC_PIN ) | _BV(LCD_CE_PIN) | _BV(SPI_MOSI_PIN) | _BV(LCD_RST_PIN) | _BV(SPI_CLK_PIN)); //Clear Bits
